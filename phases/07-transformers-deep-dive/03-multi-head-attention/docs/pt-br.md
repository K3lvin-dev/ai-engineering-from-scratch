...
Na inferência isso economiza memória porque só cópias de `n_kv_heads` ficam no KV cache, não `n_heads`. Llama 3 70B usa 64 query heads com 8 KV heads — encolhimento de cache de 8×.
...
2. **Médio.** Implemente MQA (uma KV head compartilhada pra todas as query heads). Meça quanto a contagem de parâmetros cai vs MHA completa. Calcule quanto o tamanho do KV-cache encolhe na inferência com N=2048.
