...
Um LLM de 70B amostrando um token leva ~30 ms num H100. Um modelo draft de 3B leva ~3 ms. Se deixarmos o 3B rascunhar 5 tokens à frente, e depois rodar o 70B *uma vez* pra verificar todos os 5, o total é `5×3 + 30 = 45 ms` pra até 5 tokens aceitos — contra `5×30 = 150 ms` pra geração direta. Esse é o pitch completo do speculative-decoding: troque uma quantidade extra de memória GPU (modelo draft) por latência de decode 2–4× menor.

O truque tem que preservar a distribuição. O speculative sampling, introduzido por Leviathan et al. (2023) e por Chen et al. simultaneamente, garante que a sequência de saída é **identicamente distribuída** ao que o modelo grande teria produzido sozinho. Sem tradeoff de qualidade. Só mais rápido.
...
4. **Lookahead decoding (Fu 2024).** Iteração Jacobi; não precisa de modelo draft nenhum. Auto-speculation. Nicho mas sem dependências.

Toda pilha de inferência em produção em 2026 já traz speculative decoding por padrão. vLLM, TensorRT-LLM, SGLang, e llama.cpp todos suportam pelo menos vanilla + EAGLE-2.
...
Prós: sem segundo modelo. Contras: adiciona parâmetros treináveis; precisa de uma etapa de fine-tuning supervisionado (~1B tokens); taxa de aceitação é um pouco menor que speculative vanilla com um bom draft.
...
EAGLE-3 (2025) adicionou busca em árvore sobre continuações candidatas. vLLM e SGLang trazem EAGLE-2/3 como o pathway spec padrão pra Llama 3/4 e Qwen 3.
...
Implementações em produção (o `--speculative-model` do vLLM, o LookaheadDecoder do TensorRT-LLM) lidam com isso usando buffers KV temporários. Escreve primeiro, commita na aceitação. Não é conceitualmente difícil, mas é chato.

## Construindo

Veja `code/main.py`. Implementamos o algoritmo central de speculative sampling (etapa de rejeição + distribuição residual) com:
...
Rode 10.000 passos speculative em diferentes níveis de qualidade de draft. Plote a taxa de aceitação vs. divergência KL entre distribuições draft e verificador. Você deve ver uma relação monótona limpa.

### Passo 5: verifique equivalência de distribuição

Empiricamente: o histograma de tokens produzidos pelo loop speculative deve bater com o histograma produzido por amostragem direta do verificador. Esse é o teorema de Leviathan na prática. Um teste chi-quadrado confirma dentro do erro de amostragem.
...
TensorRT-LLM tem o pathway Medusa mais rápido até meados de 2026. `faster-whisper` envolve speculative decoding pra Whisper-large com um draft pequeno.
...
**Quando NÃO usar speculative decode:**
...
Veja `outputs/skill-spec-decode-picker.md`. A skill escolhe uma estratégia de speculative decoding (vanilla / Medusa / EAGLE / lookahead) e parâmetros de ajuste (N, temperatura do draft) pra um novo workload de inferência.

## Exercícios

1. **Fácil.** Rode `code/main.py`. Confirme que a distribuição de tokens speculative bate com a distribuição de amostragem direta do verificador em 50.000 tokens com chi-square p > 0.05.
...
| Verifier | "O grande" | Modelo alvo cuja distribuição preservamos; roda uma vez por passo speculative. |
| Taxa de aceitação (α) | "Quão souvent o draft tá certo" | Probabilidade por token do verificador aceitar o draft. 0.7–0.9 típico. |
| Distribuição residual | "O fallback de rejeição" | `(q - p)_+` normalizado; amostrar disso na rejeição preserva a distribuição do verificador. |
...
| Lookahead decoding | "Iteração Jacobi" | Auto-speculation usando iteração de ponto fixo; sem modelo draft. |
...
- [vLLM docs — Speculative Decoding](https://docs.vllm.ai/en/latest/features/spec_decode.html) — referência canônica de produção com todas as quatro estratégias implementadas.
