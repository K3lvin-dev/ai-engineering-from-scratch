...
- **Fórum compartilhado.** Descobertas são publicadas em um fórum compartilhado cujo backend de armazenamento fica fora de todos os sandboxes. Agents podem ler o fórum; agentes não podem deletar ou editar registros do fórum de dentro de seu próprio sandbox.
...
A analogia em termos de infraestrutura é logs append-only com write-through para armazenamento externo (ex: object-lock S3, logs imutáveis do Datadog). O agente não tem credencial para modificar registros passados. Esse é o mesmo padrão que equipes de segurança corporativa de 2026 usam para trilhas de auditoria resistentes a ransomware.
...
|---|---|---|
| AAR | "Pesquisador de Alinhamento Automatizado" | Instância do Claude Opus 4.6 operando autonomamente em problemas de alinhamento |
...
| Fórum compartilhado | "Onde agentes publicam descobertas" | Armazenamento append-only, fora do sandbox |
...
- [Anthropic — Measuring AI agente autonomy](https://www.anthropic.com/research/measuring-agent-autonomy) — enquadramento mais amplo de autonomia de agents.
