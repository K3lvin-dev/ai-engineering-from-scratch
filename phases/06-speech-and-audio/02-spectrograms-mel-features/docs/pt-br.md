...
**Espectrograma log-mel.** `log(mel_espectrograma + 1e-10)`. A entrada do Whisper. A entrada do Parakeet. A entrada do SeamlessM4T. O frontend de áudio universal de 2026.
...
```python
def log_mel(mel_espectrograma, eps=1e-10):
    return [[math.log(max(v, eps)) for v in frame] for frame in mel_espectrograma]
```

Alternativas comuns: `librosa.power_to_db` (dB normalizado por referência), `10 * log10(power + eps)`. O Whisper usa um routine mais elaborado de clip + normalização (veja `log_mel_espectrogram` do Whisper).
...
| Log-mel | A entrada do Whisper | `log(mel_espectrograma + eps)`; padronizado em 2026. |
...
- [OpenAI — Whisper source, log_mel_espectrogram](https://github.com/openai/whisper/blob/main/whisper/audio.py) — leia a implementação de referência.
- [librosa funcionalidade extraction docs](https://librosa.org/doc/main/feature.html) — referência para `mfcc`, `melespectrogram` e hop/janela.
