...
> O Swarm da OpenAI (outubro de 2024) destilou a orquestração multi-agent em dois primitives: **routines** (instruções + ferramentas como system prompt) e **handoffs** (uma ferramenta que retorna outro Agent). Sem máquina de estados, sem DSL de branching — o LLM roteia chamando a ferramenta de handoff certa. O OpenAI Agents SDK (março de 2025) é o sucessor em produção. O Swarm em si continua sendo a referência conceitual mais limpa — o código inteiro cabe em algumas poucas centenas de linhas. O padrão é viral porque a superfície de API é mais ou menos "agent = prompt + tools; handoff = função que retorna agent." Limitação: stateless, então memória é problema de quem chama.
...
O Swarm vai na direção opusa: usa a capacidade de tool-calling que o modelo já tem. Handoffs viram chamadas de tool. O orquestrador é o agente que está segurando a conversa no momento. A máquina de estados está implícita nos prompts dos agents.

## Conceito

### Dois primitives

**Routine.** Um system prompt que define o papel do agente e as ferramentas disponíveis. Pense como um conjunto de instruções delimitado: "você é um agente de triagem; se o usuário perguntar sobre reembolsos, faça handoff para o agente de reembolso."

**Handoff.** Uma ferramenta que o agente pode chamar que retorna um novo objeto Agent. O runtime do Swarm detecta o valor de retorno Agent e troca o agente ativo para o próximo turno.
...
triage_agent = Agent(
    name="triage",
    instructions="Route the user to the right especificaçãoialist.",
    functions=[transfer_to_refunds, transfer_to_sales, transfer_to_support],
)
```

O system prompt do agente de triagem faz ele escolher o handoff certo com base na mensagem do usuário. O tool-calling do LLM faz o roteamento.
...
- **Sem ônus de máquina de estados.** Você não descreve o grafo; os prompts dos agentes descrevem pra quem eles fazem handoff.
...
- **Sessões longas com memória compartilhada.** Handoffs resetam o estado da conversa pro prompt do novo agente + histórico. Sem estado persistente entre agentes sem memória gerenciada pelo chamador.
- **Execução paralela.** Handoff é um-de-cada-vez — o agente ativo muda. Paralelismo requer o chamador orquestrando múltiplas runs do Swarm.
...
Swarm é "agent decide o que vem depois"; GroupChat é "manager decide o que vem depois." A decisão do Swarm vive na chamada de ferramenta do agente ativo; a do GroupChat vive no `GroupChatManager`.
...
`outputs/skill-handoff-designer.md` desenha uma topologia de handoff para uma tarefa dada: quais agentes existem, quais handoffs podem chamar, qual contexto é transferido.
...
1. Execute `code/main.py`, faça triagem para o agente de reembolso. Confirme que o agente ativo no segundo turno é o de reembolso.
2. Adicione uma regra de detecção de loop: se os mesmos dois agentes fizeram handoff 3 vezes seguidas, force uma saída. Projete o fallback.
3. Leia os docs do OpenAI Agents SDK sobre filtros de handoff. Implemente uma versão "resumo-no-handoff": o agente de saída comprime o contexto em um resumo antes que o agente de entrada assuma.
...
| Handoff | "Transferir para outro agent" | Uma ferramenta que o agente ativo pode chamar que retorna um novo Agent. O runtime troca o agente ativo. |
| Stateless | "Sem memória entre runs" | Swarm não persiste nada; memória é responsabilidade de quem chama. |
| Active agente | "Quem está falando agora" | O agente segurando a conversa no momento. Handoff muda isso. |
