...
Seu agente precisa chamar outro agente em outro sistema. Como? Você pode expor um endpoint HTTP, definir um schema JSON bespoke, e torcer pro outro lado falar a mesma língua. Cada par de agente vira uma integração custom.

A2A é o protocolo universal de rede pra essa chamada. Descoberta padrão, modelo de task padrão, transport padrão, artifacts padrão. Como HTTP+REST mas com agentes como cidadãos de primeira classe.
...
**Ciclo de vida opaco.** A2A não prescreve *como* o agente remoto resolve a task. O client vê transições de estado e artifacts; a implementação é livre pra usar qualquer framework.

### A divisão MCP/A2A

- **MCP** (Lição 13): agente ↔ tool. O agente lê/escreve via JSON-RPC pra um server de tools. Stateless por padrão.
- **A2A**: agente ↔ agent. Protocolo peer; ambos os lados são agentes com seu próprio raciocínio.

Sistemas multi-agent em produção usam os dois. Um peer A2A chama ferramentas MCP do seu lado. A divisão mantém as duas preocupações limpas.
...
A adoção empresarial impulsionou a escala do A2A. O destaque: A2A virou o jeito de sistemas agente empresariais cruzarem limites de confiança. O Google Cloud lançou suporte a A2A no Vertex AI Agent Builder; o Microsoft Agent Framework suporta; a maioria dos frameworks grandes (LangGraph, CrewAI, AutoGen) fornece adaptadores A2A.

### Onde o A2A vence

- **Chamadas inter-organização.** Agent na empresa A chama agente na empresa B. Sem A2A, cada par é um contrato bespoke.
- **Frameworks heterogêneos.** Agent LangGraph chama agente CrewAI chama agente Python custom. A2A normaliza.
...
- **Fixe a versão da especificação.** A2A ainda está evoluindo; o Agent Card deve declarar a versão do protocolo.
...
|-------|-------------------|--------------------------|
| A2A | "Agent-to-agent" | Protocolo peer para agentes chamarem outros agentes entre sistemas. Google 2025. |
...
| MCP vs A2A | "Tools vs peers" | MCP: vertical agente ↔ tool. A2A: horizontal agente ↔ agent. |
