...
2. **Handoff** — uma transferência estruturada de controle de um agente pra outro. Mecanicamente, uma chamada de ferramenta que retorna um novo agente ou uma aresta de grafo que segue uma condição.
3. **Estado compartilhado** — qualquer estrutura de dados que mais de um agente possa ler (às vezes escrever). Message pool, blackboard, key-value store, memória vetorial.
...
|-----------|-------|---------|----------------------|--------------|
| OpenAI Swarm / Agents SDK | `Agent(instructions, tools)` | ferramenta retorna Agent | problema de quem chama | próxima chamada de handoff do LLM |
...
Sem memória. Sem estado. Dois agentes com o mesmo system prompt e ferramentas são intercambiáveis. Tudo que parece estado por agente é na verdade estado compartilhado ou protocolo de handoff.
...
- **Retorno de função** — a ferramenta retorna o próximo agent. Esse é o padrão OpenAI Swarm. Agents carregam o roteamento nos seus schemas de tool.
...
Duas topologias: **pool completo** (cada agente vê cada mensagem) e **projetado** (agents veem uma visão limitada por papel). Pools completos são simples e escalam mal. Pools projetados escalam mas exigem design de schema antecipado.
...
- **Orientado a handoff** — o agente atual decide chamando uma ferramenta de handoff (Swarm).
...
`code/main.py` implementa as quatro primitivas em ~150 linhas de Python stdlib. Sem LLM real — cada agente é uma política scriptada pra que o foco fique na estrutura de coordenação.
...
A demo roda o mesmo pipeline de três agentes (pesquisa → escrita → review) nos três tipos de orquestrador e imprime o message pool no final. Você vê que as saídas diferem só em *quem escolhe a seguir*; os agentes e o estado compartilhado são idênticos entre execuções.
...
Saída esperada: três execuções de orquestrador, uma por padrão. Cada uma imprime o message pool final. A execução orientada a handoff alcança menos agentes se o pesquisador decide que acabou cedo — esse é o tradeoff de roteamento por LLM em miniatura.
...
1. Execute `code/main.py` três vezes com diferentes políticas de agent. Observe como a escolha do orquestrador muda quais agentes rodam.
2. Implemente um quarto tipo de orquestrador: orientado a fila onde agentes fazem polling no estado compartilhado por trabalho. Que deadlock pode acontecer, e como você detecta?
...
5. Encontre um framework nesta tabela que esconda completamente o estado compartilhado. Explique o que quebra quando agentes precisam coordenar entre handiffs sem reler o histórico.
...
| Handoff | "Transferência de controle" | Uma chamada estruturada que nomeia o próximo agente e payload opcional. Três implementações: retorno de função, aresta de grafo, seleção de falante. |
...
- [Workflows e agentes LangGraph](https://docs.langchain.com/oss/python/langgraph/workflows-agents) — orquestração por aresta de grafo e estado compartilhado baseado em reducer
- [Introdução ao CrewAI](https://docs.crewai.com/en/introduction) — agentes de papel-objetivo-história, processos Sequential / Hierarchical
