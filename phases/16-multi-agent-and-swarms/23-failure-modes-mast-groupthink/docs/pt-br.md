...
**Problemas de Especificação (41.77% das falhas).** A tarefa do agente não foi definida estritamente o suficiente. Exemplos:

- Ambiguidade de papel: dois agentes acham que são o revisor.
- Tarefa subespecificaçãoificada: "resuma isso" quando o usuário queria um ângulo específico.
...
### STRATUS — agentes especializados pra detecção de falhas
...
Isso é resposta a incidente no estilo SRE, aplicada a sistemas de agent. Os três papéis podem ser todos LLM agentes com prompts especializados.
...
| Problema de Especificação | "Ambiguidade de papel" | Tarefa ou papel sub-definido; agentes não sabem o que fazer. |
...
- [STRATUS — agentes especializados pra resposta a incidentes em MAS](https://neurips.cc/) — proceedings NeurIPS 2025 (detecção + diagnóstico + validação)
