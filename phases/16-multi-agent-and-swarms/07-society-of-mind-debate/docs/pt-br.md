...
Debate quebra a saturação. Em vez de N amostras independentes de um modelo, N agentes leem o raciocínio uns dos outros e revisam. A correlação entre amostras cai (elas não são mais i.i.d.), e o ponto de convergência é frequentemente correto onde a votação i.i.d. estava confiantemente errada.
...
1. Cada um dos N agentes produz uma resposta inicial pra pergunta.
2. Pra rodada r = 2..R: cada agente é mostrado as respostas dos outros agentes da rodada r-1 e pedido "considerando essas, dê sua resposta atualizada."
...
- **Contagem de agentes sozinha** (1 rodada, votação por maioria de N) supera single-agent na maioria das tarefas, mas estagna.
- **Contagem de rodadas sozinha** (1 agente vendo seu próprio raciocínio anterior) quase não ajuda — fraqueza conhecida da reflexão.
- **As duas juntas** produzem os grandes saltos. A troca multi-rodada entre múltiplos agentes impulsiona o ganho.

### Por que funciona

Dois mecanismos:

1. **Exposição à discordância.** Quando um agente vê a cadeia de raciocínio de outro agente com uma conclusão diferente, ele tem que justificar ou atualizar. De qualquer forma, o contexto pra rodada r+1 é mais rico que o da rodada r.
...
- **Cascata de subserviência.** Todos os agentes cedem pro agente que soa mais confiante. O debate colapsa pra voz mais alta. Pedir papéis adversariais ("um agente deve argumentar a posição contrária") ajuda.
- **Deriva de tópico.** Debates por muitas rodadas derivam da pergunta original. Mitigação: reinjete a pergunta a cada rodada.
- **Explosão de compute.** N agentes × R rodadas = N·R chamadas LLM, cada uma com contexto que cresce. Um debate de 5 agents, 5 rodadas são 25 chamadas com contexto crescente. Custo por pergunta pode exceder 10× uma chamada CoT.

## Construa

`code/main.py` roda um debate de 3 agentes × 3 rodadas numa pergunta de matemática onde cada agente começa com uma resposta diferente (possivelmente errada). Agents são scriptados — cada um "atualiza" fazendo média das respostas dos vizinhos ponderadas por uma confiança scriptada. Convergência é visível no log rodada a rodada.

A demo mostra dois efeitos chave:

- Uma rodada única de troca aproxima agentes da resposta correta.
...
- **Limite agentes a 5.** Além de 5, expansão de contexto e custo dominam.
- **Heterogêneo por padrão.** Pelo menos dois modelos base diferentes no pool.
- **Slot adversarial.** Um agente promptado pra discordar sempre. Quebra subserviência.
...
2. Adicione um quarto agente com papel adversarial: sempre discorda da maioria atual. Isso quebra ou melhora a convergência?
3. Plote (imprima) o score de concordância por rodada (fração de agentes na resposta da maioria). Quando ele atinge 1.0 e isso é equivalente a "correto"?
...
| Cascata de subserviência | "Todos concordam com o barulhento" | Falha de debate onde agentes cedem pro agente mais confiante independente de correção. |
