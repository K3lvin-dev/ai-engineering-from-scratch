...
Karras et al. (2019, NVIDIA) propuseram: pare de alimentar `z` diretamente nas camadas conv. Alimente um tensor constante `4×4×512` como input da rede. Aprenda um MLP de 8 camadas que mapeia `z ∈ Z → w ∈ W`. Injete `w` em cada resolução via *normalização de instância adaptativa* (AdaIN): normaliza cada mapa de características de conv, depois escala e desloca com projeções afins de `w`. Adiciona ruído por camada para detalhes estocásticos (poros de pele, fios de cabelo).
...
onde `y_scale` e `y_bias` vêm de projeções afins de `w`. Normaliza por mapa de características, depois reestiliza. "Estilo" aqui são as estatísticas de primeira e segunda ordem do mapa de características.

**Ruído por camada.** Ruído Gaussiano de canal único adicionado a cada mapa de características, escalado por um fator por canal aprendido. Controla detalhes estocásticos sem afetar estrutura global.
...
Scale e bias por mapa de características vêm de `w` via projeção linear.
...
- **Artefatos de gota.** StyleGAN 1 produzia uma gota borrada nos mapa de característicass porque AdaIN zerava a média. A demodulação de pesos do StyleGAN 2 corrige escalando os pesos de convolução no lugar.
...
| AdaIN | "Normalização de instância adaptativa" | Normaliza mapa de características, depois escala + desloca por projeção de `w`. |
