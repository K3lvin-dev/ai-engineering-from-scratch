...
Esta aula produz `outputs/skill-token-gen-cost-analyzer.md`. Dada uma especificação de produto de geração (imagem ou vídeo, resolução alvo, tier de qualidade, orçamento de latência), calcula contagens de tokens, custo de inferência e escolhe família Emu3 vs difusão.
