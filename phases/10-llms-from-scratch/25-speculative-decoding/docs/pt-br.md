...
Decodificacao especulativa e **provavelmente equivalente em distribuicao a amostrar de p**. A regra de rejeicao:
...
Li, Wei, Zhang, Zhang (2024, "EAGLE: Speculative Sampling Requires Rethinking Feature Uncertainty") observaram duas ineficiencias na decodificacao especulativa padrao:
...
- Uma referencia `speculative_decode(target, draft, prompt, K, temperature)` que implementa a regra exata de rejeicao e verifica que preserva a distribuicao do alvo (KL empirico < 0.01 vs amostragem simples do alvo).
...
def speculative_step(p_target, q_draft, K, temperature=1.0):
    """Uma rodada de decodificacao especulativa. Retorna lista de tokens aceitos."""
...
- **vLLM** e **SGLang** enviam decodificacao especulativa de primeira classe. Flags: `--speculative_model`, `--num_speculative_tokens`. Suporte EAGLE-2/3 via flag `--spec_decoding_algorithm eagle`.
- **NVIDIA TensorRT-LLM** suporta Medusa e arvores EAGLE nativamente.
- **Modelos de rascunho de referencia**: `Qwen/Qwen3-0.6B-spec` (rascunhos pra Qwen3-32B), `meta-llama/Llama-3.2-1B-Instruct-spec` (rascunhos pra 70B).
...
Esta aula produz `outputs/skill-speculative-tuning.md` -- uma skill que profileia a carga de trabalho de um modelo alvo e escolhe: modelo de rascunho, K (comprimento do rascunho), largura da arvore, temperatura e quando voltar pra decodificacao simples.

## Exercicios

1. Implementar a regra exata de rejeicao e verificar empiricamente. Rodar 10K amostras via `speculative_decode` e via amostragem simples do alvo; calcular distancia TV entre as duas distribuicoes de saida. Deve ser < 0.01.
...
5. Medir modos de falha. Rodar decodificacao especulativa em temperatura=1.5 (alta estocasticidade). Mostrar que alpha despenca e o algoritmo e mais lento que decodificacao simples por causa do overhead do rascunho.
...
| Modelo de rascunho | "O especulador" | O preditor menor, mais rapido (distribuicao q); 5-30x menor |
| K / comprimento do rascunho | "Olhar adiante" | Numero de tokens especulados por passo de verificacao |
...
| Arvore de rascunho | "Especulacao ramificada" | Rascunho produz uma arvore de candidatos, verificada em uma passada com mascara de atencao estruturada em arvore |
...
- [Chen, Borgeaud, Irving et al., 2023 -- "Accelerating Large Language Model Decoding with Speculative Sampling"](https://arxiv.org/abs/2302.01318) -- paper concorrente de amostragem especulativa no DeepMind
...
- [Fu, Haotian, Peng et al., 2024 -- "Break the Sequential Dependency of LLM Inference Using Lookahead Decoding"](https://arxiv.org/abs/2402.02057) -- decodificacao Jacobi/lookahead, alternativa sem especulador
