...
Sentimento é um laboratório funcionando pra NLP clássico. Se você entende por que cada baseline ingênuo tem um modo de falha específico, entende por que cada modelo mais rico foi inventado. Essa lição constrói um baseline Naive Bayes do zero, adiciona regressão logística, e nomeia as armadilhas que fazem sentimento de produção ser um problema de nível de conformidade.
...
def apply_negation(tokens):
    ...
    for token in tokens:
        ...
        if token in NEGATION_WORDS:
            ...
            out.append(token)
            continue
        out.append(f"NOT_{token}" if negue else token)
    return out
```
...
- **Matriz de confusão.** Contagens brutas. Sempre inspecione antes de confiar em qualquer métrica escalar; revela qual par de classes o modelo confunde.
...
- Sentimento baseado em aespecificaçãoto. "Camera was great but battery was terrible." Você precisa atribuir sentimento a aaspectos. Só transformers ou modelos de saída estruturada.
...
| Sentimento baseado em aespecificaçãoto | Polaridade por aespecificaçãoto | Atribui sentimento a entidades ou atributos específicos mencionados no texto. |
