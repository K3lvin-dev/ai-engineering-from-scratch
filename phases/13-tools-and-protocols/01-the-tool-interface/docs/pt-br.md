...
- Desenhar o loop de quatro passos de tool call (descrever → decidir → executar → observar) e nomear quem é dono de cada passo.
...
O executor em si é código comum. Python, TypeScript, um comando shell, uma query de banco. Ele produz um resultado, que geralmente é uma string mas pode ser qualquer valor JSON ou um bloco de conteúdo estruturado (texto, imagem ou referência de recurso no MCP). O resultado deve ser serializável.
...
Fase 14 · 12 cobrecuperação de erros e auto-healing em profundidade; Fase 17 cobre rate limits de produção.
...
- O registry de ferramentas mantém três campos por ferramenta: nome, descrição, schema e uma referência do executor.
...
| Parallel tool calls | "Muitas chamadas de uma vez" | Múltiplos objetos de chamada num turno do modelo, independentes e ordenáveis por id |
...
| Tool consecutiva | "Ferramenta de ação" | Muta estado externo; requer gate, auditoria ou confirmação do usuário |
