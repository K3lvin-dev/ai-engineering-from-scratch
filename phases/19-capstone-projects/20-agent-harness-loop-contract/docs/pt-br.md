...
- Especificaçãoificar um agente harness loop como uma máquina de estados determinística com transições explícitas.
...
- Emitir um stream tipado de onze tipos de eventos para que UIs e tracers downstream possam se especificar sem especificar o loop diretamente.
...
Um ponto de pull não é uma exceção. É um return. O caller especifica o estado do harness, busca o que o harness pediu, e chama `resume(payload)`. O harness retoma de onde parou. Essa é a mesma forma de um Python generator. O transport sobre o ponto de pull é sua escolha. Em uma TUI é tecla pressionada. Via MCP é `tools/call`. Via fila é um job poll.
