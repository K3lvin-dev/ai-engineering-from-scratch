...
Papéis são agentes tipados. **Arquiteto** (Claude Opus 4.7) lê a issue, escreve um plano e o divide em subtarefas com interfaces explícitas. **Programadores** (Claude Sonnet 4.7, N instâncias paralelas, cada um em um `git worktree` + sandbox Daytona) implementam subtarefas independentemente. **Revisor** (GPT-5.4) lê o diff mesclado e aprova ou solicita alterações específicas. **Testador** (Gemini 2.5 Pro) roda a suíte de testes isoladamente e reporta pass/falha com artefatos.
...
| Protocolo A2A | "Agent-to-agent" | Especificação de 2025 do Google para mensagens tipadas entre agentes |
