...
> Um modelo de 8B treinado nos seus próprios dados, alinhado com DPO nas suas próprias preferências, quantizado, com especulativa decoding e servido com $/1M tokens mensuráveis. O stack aberto de 2026 é Axolotl v0.8, TRL 0.15, Unsloth para iteração, GPTQ/AWQ/GGUF para quantização, vLLM 0.7 com EAGLE-3 para serving. O capstone é rodar a pipeline inteira de forma reproduzível — YAML de entrada, endpoint servido na saída — e publicar um model card sob o Model Openness Framework de 2026.
...
quantização GPTQ + AWQ + GGUF
    |
    v
vLLM 0.7 + EAGLE-3 especulativa decoding
    |
    v
...
- Serving: vLLM 0.7 com EAGLE-3 especulativa decoding (ou SGLang 0.4 + SpecForge)
...
6. **Sirva com especulativa decoding.** Config do vLLM 0.7 com cabeçalhos EAGLE-3 treinados via Red Hat Speculators. Meça taxa de aceitação e latência de cauda em batch 1 / 8 / 32. Relate $/1M tokens vs Anthropic / OpenAI na mesma avaliação.
...
| EAGLE-3 | "Draft de especulativa decoding" | Cabeçalhos draft que predizem N tokens à frente; vLLM verifica com o modelo alvo |
...
- [SGLang SpecForge](https://github.com/sgl-project/SpecForge) — treinador de especulativa decoding alternativo
