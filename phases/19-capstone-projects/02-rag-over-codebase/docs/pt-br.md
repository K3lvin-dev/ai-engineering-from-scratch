        ...
        (Voyage / bge)  (Tantivy)        (Haiku 4.5)
            ...
            +------> Qdrant / pgvector <----+
                      ...
                      grafo de símbolos (Neo4j / kuzu)
                            |
  query --> agent LangGraph (recuperar -> re-rankear -> sintetizar)
                            |
                            v
...
- Embeddings densos: Voyage-code-3 (hospedado) ou nomic-embed-code-v1.5 (auto-hospedado), fallback bge-code-v1
...
- Orquestração: LlamaIndex Workflows para ingestão, LangGraph para agent de query
...
5. **Grafo de símbolos.** Para cada chunk, registre arestas: imports (este arquivo usa o símbolo Y do repo Z), chamadas (esta função chama o método M na classe C), herança. Armazene no kuzu. Usado no momento da query para expandir a recuperação além das fronteiras de repo.

6. **Agent de query.** LangGraph com três nós. `retrieve` dispara densa + BM25 em paralelo, deduplica por (repo, path, símbolo). `rerank` roda o cross-encoder no top-50 e mantém top-10. `synth` chama Claude Sonnet 4.7 com os chunks re-rankeados em contexto, faz cache do prompt do sistema, exige citações arquivo:linha.
...
A skill entregável é `outputs/skill-codebase-rag.md`. Dado um corpus de repos, ela monta a pipeline de ingestão, o índice híbrido e o agent de query, e retorna uma resposta citada para qualquer questão cross-repo. Rubrica:
...
| 20 | Latência e escala | p95 da latência de query em 10k QPS no corpus indexado |
...
4. Adicione uma verificação de drift baseada em amostragem: semanalmente, reexecute a avaliação de 100 questões. Alerta em queda de MRR@10 > 5%.
...
| Re-ranqueamento cross-encoder | "Ranqueamento de segunda etapa" | Modelo que pontua cada par (query, candidato) junto, mais preciso que cosseno |
