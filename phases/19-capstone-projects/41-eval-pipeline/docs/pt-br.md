...
Cada eval e uma funcao `(model, dataset) -> EvalResult`. O resultado carrega o valor da metrica, detalhes por exemplo para inspeção, e um nome para o agregado. O pipeline compoe eles com uma config que diz quais evals rodar e como pondera-los.
