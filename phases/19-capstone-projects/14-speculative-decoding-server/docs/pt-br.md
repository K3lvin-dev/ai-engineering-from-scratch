...
- Métodos especificaçãoulativos: cabeçalhos draft EAGLE-3, especulação paralela P-EAGLE, reserva ngram
...
- Referência: TensorRT-LLM especulativa decoding como baseline de fornecedor
...
2. **Fonte do draft.** Puxe um cabeçalho draft EAGLE-3 alinhado do Red Hat Speculators (ou treine um via SpecForge). Carregue na config de especulativa decoding do vLLM.
...
`outputs/skill-inference-server.md` descreve a entrega. Um stack de serving medido com especulativa decoding, um relatório de teste completo e um implantação K8s.
...
5. Teste TensorRT-LLM especulativa decoding no mesmo hardware H100. Relate onde ele ganha vs vLLM.
...
| P-EAGLE | "Especificaçãoulação paralela" | Árvore de ramificações draft verificadas em um único passo do alvo |
...
- [P-EAGLE (AWS 2026)](https://aws.amazon.com/blogs/machine-learning/p-eagle-faster-llm-inference-with-parallel-especulativa-decoding-in-vllm/) — paper de especulativa decoding paralelo + integração
