...
> EAGLE-3 no vLLM 0.7 entrega 2.5-3x de throughput em tráfego real. P-EAGLE (AWS 2026) empurrou especulação paralela ainda mais. SpecForge do SGLang treinou cabeçalhos draft em escala. Hub Speculators do Red Hat publicou drafts alinhados para modelos abertos comuns. TensorRT-LLM tornou speculative decoding de primeira classe na NVIDIA. O stack de serving de produção de 2026 é vLLM ou SGLang com drafts da família EAGLE, quantização FP8 ou INT4 e HPA em queue-wait. Este capstone é servir dois modelos abertos a 2.5x+ do throughput baseline com um relatório completo de latência de cauda.
...
EAGLE-3 supera drafts ngram na maioria dos tráfegos. P-EAGLE roda especulação paralela para árvores draft mais profundas. O trade-off: p99 de latência na rejeição é maior porque o passo de verificação é maior. A config de serving precisa reportar latência agrupada por tamanho de batch para revelar isso.
...
servidor vLLM (0.7) ou SGLang (0.4)
    |
    +-- draft: cabeçalhos EAGLE-3 | P-EAGLE paralelo | fallback ngram
...
- Métodos especulativos: cabeçalhos draft EAGLE-3, especulação paralela P-EAGLE, fallback ngram
...
- Referência: TensorRT-LLM speculative decoding como baseline de fornecedor
...
2. **Fonte do draft.** Puxe um cabeçalho draft EAGLE-3 alinhado do Red Hat Speculators (ou treine um via SpecForge). Carregue na config de speculative decoding do vLLM.

3. **Números baseline.** Antes da especulação: tokens/s em batch 1/8/32, latência p50/p99, utilização de GPU. Publique.
...
5. **P-EAGLE.** Habilite especulação paralela; meça árvore draft mais profunda vs EAGLE-3 serial. Relate o ponto de inflexão onde P-EAGLE ajuda vs prejudica.

6. **Tráfego de domínio.** Rode ShareGPT vs HumanEval vs tráfego específico de domínio pelo mesmo servidor. Meça taxa de aceitação por distribuição. Identifique quando os drafts divergem.
...
`outputs/skill-inference-server.md` descreve a entrega. Um stack de serving medido com speculative decoding, um relatório de teste completo e um deploy K8s.
...
| 20 | Disciplina de p99 de latência de cauda | p99 em batch 1/8/32 com e sem especulação |
...
1. Meça a degradação da taxa de aceitação quando o draft está uma versão atrás do alvo (ex.: drift de Llama 3.3 -> 3.4). Construa um alerta de monitoramento.

2. Implemente fallback ngram: se a aceitação do EAGLE-3 cair abaixo de um limiar, troque para drafts ngram. Relate melhoria na confiabilidade.
...
5. Teste TensorRT-LLM speculative decoding no mesmo hardware H100. Relate onde ele ganha vs vLLM.
...
| P-EAGLE | "Especulação paralela" | Árvore de ramificações draft verificadas em um único passo do alvo |
...
- [P-EAGLE (AWS 2026)](https://aws.amazon.com/blogs/machine-learning/p-eagle-faster-llm-inference-with-parallel-speculative-decoding-in-vllm/) — paper de speculative decoding paralelo + integração
- [SGLang SpecForge](https://github.com/sgl-project/SpecForge) — pipeline de treinamento de cabeçalho draft
- [Red Hat Speculators](https://github.com/neuralmagic/speculators) — hub de drafts alinhados
