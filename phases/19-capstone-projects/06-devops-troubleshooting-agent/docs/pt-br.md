...
- Deploy: deploy K8s com sua própria role RBAC estreita; namespace separado
...
3. **Superfície de ferramentas somente-leitura.** Envolta de kubectl, query do Prometheus, logql do Loki, traceql do Tempo via FastMCP. Cada ferramenta tem um verbo RBAC estreito ("get", "list", "describe"). Nenhum "delete", "exec", "scale" no servidor padrão.
...
5. **Pontuação de evidência.** Cada hipótese tem pontuação = recência * especificidade * inverso do comprimento do caminho no grafo * contagem de citações. Retorne top-3.
...
7. **Portão de remediação.** Ferramentas destrutivas (escalar para baixo, rollback, delete) ficam num segundo servidor MCP atrás de um token de aprovação. O agent só pode chamá-las após o card Slack ser aprovado por um humano.
...
9. **Conjunto de incidentes sintéticos.** Construa 20 cenários: cascata de OOMKill, instabilidade de DNS, thrash de HPA, preenchimento de PVC, vizinho barulhento, sidecar defeituoso, rollout ruim de ConfigMap, rotação de certificado, backoff de pull de imagem, etc. Pontue o agent na acurácia de causa raiz e tempo-até-hipótese.
...
[slack]   [FAZER ROLLBACK para v2.40]  [ESCALAR]  [IGNORAR]
          (aprovação necessária; agent não faz rollback unilateralmente)
```

## Entregue

`outputs/skill-devops-agent.md` é a entrega. Dado um cluster K8s e uma fonte de alerta, o agent produz hipóteses ranqueadas de causa raiz e um fluxo de remediação controlado pelo Slack.
...
1. Rode seu agent nos mesmos três incidentes que o DevOps Agent da AWS demonstra. Publique a comparação lado a lado. Relate onde o agent diverge.

2. Adicione uma auditoria de "quase-acerto" que marque qualquer comando que o agent *considerou* que seria destrutivo sem aprovação. Meça a taxa de quase-acertos em uma semana.
...
| Somente-leitura por padrão | "RBAC escopado" | Conta de serviço do agent só tem verbos get/list/describe; verbos destrutivos ficam num servidor separado atrás de aprovação |
| Log de auditoria | "Considerado vs executado" | Registro append-only de cada comando candidato, se rodou, quem aprovou |
| Ranqueamento de hipóteses | "Pontuação de evidência" | Recência × especificidade × inverso do comprimento do caminho no grafo × contagem de citações |
| Card de aprovação Slack | "Portão HITL" | Mensagem Slack interativa com botões de remediação; agent não pode prosseguir até um humano clicar |
| Citação de telemetria | "Ponteiro de evidência" | Uma query do Prometheus, seletor do Loki ou URL de trace do Tempo que sustenta uma afirmação |
...
- [LangGraph](https://langchain-ai.github.io/langgraph/) — orquestrador de agent de referência
