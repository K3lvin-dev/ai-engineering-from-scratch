...
Toda equipe de IA rodando tráfego de produção em 2026 mantém um plano de observabilidade ao lado do modelo. Atribuição de custo. Detecção de alucinação. Monitoramento de drift. Sinal de jailbreak. Paineis de SLO. Alertas de vazamento de PII. As referências open-source — Langfuse, Phoenix, OpenLLMetry — convergiram nas convenções semânticas do OpenTelemetry GenAI como schema de ingestão. Você pode agora instrumentar OpenAI, Anthropic, Google, LangChain, LlamaIndex e vLLM com um único SDK e enviar spans compatíveis.

Você vai construir um painel auto-hospedado que ingere de pelo menos quatro famílias de SDK, roda um pequeno conjunto de jobs de avaliação sobre traces amostrados, detecta drift e alerta. A barra de medição: dada uma regressão deliberadamente injetada (um prompt que começa a produzir PII), o painel pega e dispara um alerta em menos de cinco minutos.
...
Detecção de drift observa distribuições de embeddings ao longo do tempo (divergência PSI ou KL em embeddings de prompt) mais tendências de pontuação de avaliação. Alertas alimentam Prometheus Alertmanager e depois Slack / PagerDuty. A UI é Next.js 15 com Recharts.
...
apps de produção:
  ...
  coletor (ingestão, amostragem, fan-out)
   ...
   (spans)       (metadados) (eventos brutos)
       ...
       |
       +---> detector de drift (PSI / KL em embeddings de prompt)
       |
...
6. **Detecção de drift.** Job semanal que computa PSI entre os embeddings de prompt agrupados desta semana e a linha de base das 4 semanas anteriores. Se PSI acima do limiar, alerta.

7. **Painel.** Next.js 15 com páginas: visão geral (spans/seg, custo/usuário, latência p95), traces (busca + waterfall), avaliações (tendência de fidelidade, toxicidade), drift (PSI ao longo do tempo), alertas.
...
[drift]      PSI semanal 0.08 (abaixo do limiar 0.2)
[ui]         ao vivo em https://obs.example.com
```

## Entregue

`outputs/skill-llm-observability.md` é a entrega. Dada uma aplicação LLM, o painel ingere seus traces, roda avaliações, alerta em drift e exibe breakdown de custo/usuário no Next.js.
...
| 15 | Alertas + detecção de drift | Cadeia Prometheus/Alertmanager exercitada ponta a ponta |
| **100** | | |
...
3. Afine o detector de drift: compute PSI por app-id em vez de global. Mostre trilhas de drift por app.
...
| PSI | "Índice de estabilidade populacional" | Métrica de drift comparando duas distribuições; > 0.2 geralmente sinaliza drift significativo |
...
- [Arize Phoenix](https://github.com/Arize-ai/phoenix) — referência alternativa com forte suporte a drift
