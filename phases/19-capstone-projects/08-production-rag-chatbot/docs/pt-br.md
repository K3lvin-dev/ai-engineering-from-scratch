...
> Harvey, Glean, Mendable e LlamaCloud todos rodam a mesma forma de produção em 2026. Ingestão com docling ou Unstructured e ColPali para visuais. Busca híbrida. Re-ranqueamento com bge-reranker-v2-gemma. Síntese com Claude Sonnet 4.7 usando prompt caching a 60-80% de hit rate. Proteção com Llama Guard 4 e NeMo Guardrails. Monitoramento com Langfuse e Phoenix. Avaliação com RAGAS num conjunto dourado de 200 questões. Construa um num domínio regulado (jurídico, clínico, seguro) e o capstone é passar no conjunto dourado, no red team e no painel de drift.
...
parse docling / Unstructured + ColPali para visuais
      |
      v
chunks + resumos + labels-de-função + tags de jurisdição
      |
      v
pgvector + pgvectorscale  +  BM25 (Tantivy)
      |
query + função + jurisdição
      |
      v
...
- LLM: Claude Sonnet 4.7 com prompt caching; fallback Llama 3.3 70B auto-hospedado
- Avaliação: RAGAS 0.2 online, DeepEval para suites de alucinação e jailbreak
- Observabilidade: Langfuse auto-hospedado com fila de anotação; Arize Phoenix para drift
- Guardrails: classificador de entrada/saída Llama Guard 4, política NeMo Guardrails v0.12, limpeza PII Presidio
- Compliance: labels de acesso baseado em função nos chunks; tags de jurisdição para GDPR/HIPAA

## Construa

1. **Ingestão.** Parse seu corpus (1000-10000 documentos para uma implementação séria) com Unstructured ou docling. Para páginas escaneadas / pesadas em visuais, roteie pelo ColPali. Produza chunks com resumos, labels-de-função, tags de jurisdição.
...
4. **Sintetize com prompt caching.** Prompt do sistema + políticas estáticas no cabeçalho do cache; contexto re-rankeado como extensão do cache; pergunta do usuário como sufixo não-cacheado. Meta de 60-80% hit rate do cache em estado estacionário.
...
6. **Conjunto dourado.** 200 pares Q/A rotulados por um especialista do domínio com (resposta, citações). Pontue o agent em correspondência exata de citação, corretude da resposta, fidelidade (RAGAS).
...
8. **Painel de drift.** Arize Phoenix rastreia qualidade de recuperação (nDCG, fidelidade de citação) semanalmente. Alerta em queda de 5%.

9. **Relatório de custo.** Langfuse: hit rate de prompt caching, tokens por query, breakdown de $/query por estágio.
...
`outputs/skill-production-rag.md` descreve a entrega. Um chatbot de domínio regulado implantado com labels de compliance, passando pela rubrica, monitorado com drift ao vivo.
...
| 20 | Custo / engenharia de latência | Hit rate de prompt cache, p95 de latência, $/query |
| 15 | Painel de monitoramento de drift | Dashboard ao vivo Phoenix com tendência semanal de qualidade de recuperação |
| **100** | | |
...
2. Meça o hit rate de prompt cache em uma semana de tráfego de produção. Identifique quais queries quebram o prefixo do cache. Reestruture.

3. Adicione memória multi-turn com um buffer de resumo de 10k tokens. Meça se a fidelidade cai conforme a conversa cresce.

4. Troque Claude Sonnet 4.7 por Llama 3.3 70B auto-hospedado. Meça $/query e delta de fidelidade.

5. Adicione um modo "não tenho certeza": se as pontuações re-rankeadas do top estão abaixo de um limiar, o agent diz "não tenho citações confiantes" em vez de responder. Meça a redução de falsa confiança.
...
| Tag de jurisdição | "Label de compliance" | Escopo GDPR/HIPAA/SOC2 anexado aos chunks; aplicado por filtro de recuperação |
...
- [Arize Phoenix](https://github.com/Arize-ai/phoenix) — observabilidade de drift de referência
