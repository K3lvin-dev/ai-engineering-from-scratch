...
**Vertex** usa atribuição projeto-por-equipe mais rótulos em tudo. Você modela cada equipe como um projeto GCP, coloca rótulos em cada recurso e usa o BigQuery Billing Export + DataStudio para rollups. Mais trabalho, mas o BigQuery dá SQL arbitrário sobre os dados de custo.
...
Vertex: HIPAA, GDPR, residência de dados por região; stack de conformidade do Google Cloud.
...
Esta aula produz `outputs/skill-managed-platform-picker.md`. Dado um perfil de workload (modelos necessários, SLA de TTFT, volume diário, requisitos de conformidade), recomenda uma plataforma principal, uma de reserva e um plano de instrumentação de FinOps.
