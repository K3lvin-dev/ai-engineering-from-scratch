...
> O agente que escreveu o código não pode avaliá-lo. Um reviewer é um segundo loop com um system prompt diferente, um objetivo diferente, e acesso somente leitura a tudo que o construtor produziu. A distância entre construtor e reviewer é onde mora a maior parte da confiabilidade.
...
Você pede pro agente pra corrigir um bug. Ele edita quatro arquivos, roda os testes e reporta "feito". O gate de verificação (Fase 14 · 38) confirma que a aceitação rodou e que o escopo se manteve. O gate diz `passed: true`. Você faz merge. Dois dias depois descobre que a correção resolveu a metade errada do bug.
...
- Fase 14 · 05 — Self-Refine e CRITIC (baseline de autoavaliação com agente único)
