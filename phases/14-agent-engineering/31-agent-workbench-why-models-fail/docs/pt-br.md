...
- Nomear as sete superfícies do workbench que decidem se um agente faz release.
...
Isso não é um bug do modelo. É um bug do workbench. A superfície ao redor do agente tá faltando as partes que transformam uma geração one-shot em engenharia confiável e retomável.
...
Prompting diz ao modelo o que você quer neste turno. Um workbench diz ao modelo como trabalhar entre turnos e entre sessões. A maioria das histórias de falha de agente são falhas de workbench vestidas de prompt engineering.

### Workbench versus framework

Um framework te dá um runtime (LangGraph, AutoGen, Agents SDK). Um workbench dá ao agente um lugar pra trabalhar dentro desse runtime. Você precisa dos dois. Esse mini-track é sobre o segundo.
...
Tira o label de agente por um momento. Uma execução de agente é computação que cruza tempo, processos e máquinas. Pra tornar isso confiável você precisa das mesmas primitivos que qualquer sistema de produção precisa.

| Primitivo | O que é | O que carrega pra um agente |
|-----------|---------|---------------------------|
...
| Trigger | Fonte de eventos que invoca uma function | Tick do agente loop, requisição HTTP, mensagem de fila, cron, mudança de arquivo, hook |
...
O agente loop em si é um worker que consome eventos (mensagem do usuário, resultado de ferramenta, tick de timer), chama functions (o modelo, depois as ferramentas que o modelo escolhe), grava registros (estado, feedback) e emite triggers (verify, review, handoff). Sem segredos; a mesma forma de um job processor.
...
|--------------------------------|-------------------|
| Ralph Loop (Claude Code, Codex, livro agentic_harness) — reinjeta a intenção original numa janela de contexto nova quando o agente tenta parar cedo | Um trigger que re-enfileira uma tarefa com contexto limpo; a persistência de sessão carrega o objetivo adiante |
| Plan / Execute / Verify (PEV) | Três workers, um por papel, comunicando via estado e uma queue entre fases |
| Separação harness-compute (OpenAI Agents SDK, abril 2026) — separa plano de controle de plano de execução | Reformulação de plano de controle / plano de dados. Antecede o label de agente por décadas |
...
| Sandbox agentes (Codex, Sandcastle, Vercel Sandbox) | O plano de computação: um runtime com sistema de arquivos, rede e ciclo de vida isolados |
| MCP servers | Workers que expõem functions via uma RPC estável, com listas de capacidades como autorização |

Cada entrada nessa tabela é a comunidade de agente chegando numa primitivo que já tinha nome em sistemas distribuídos e dando um novo nome. Útil pra marketing; não útil como vocabulário de engenharia.
...
- Terminal Bench 2.0 — mesmo modelo, mudança de harness movimentou um agente de codificação de fora do top 30 pra quinto lugar (LangChain, *Anatomy of an Agent Harness*).
- Vercel — deletou 80% das ferramentas do seu agent; taxa de sucesso pulou de 80% pra 100% (MongoDB).
- Harvey — agentes legais mais que dobraram a precisão só com otimização de harness (MongoDB).
- 88% dos projetos de AI agente empresariais falham em chegar a produção. As falhas se concentram em runtime, não em raciocínio (preprints.org, *Harness Engineering for Language Agents*, março 2026).
...
O agente é um stub baseado em regras bem pequeno; o ponto são as superfícies, não o modelo. No resto desse mini-track você vai reconstruir cada superfície como um artefato real e reutilizável.
...
4. Rode o script novamente com um stub agente diferente que alucina uma escrita extra de arquivo. Qual superfície pega primeiro?
...
| Superfície | "Um doc" ou "um script" | Um input nomeado e legível por máquina que o agente lê ou escreve a cada turno |
| Sistema de registro | "As anotações" | O arquivo que o agente trata como verdade quando o histórico de chat acabou |
| Definição de pronto | "Aceitação" | Uma checklist objetiva, respaldada em arquivo, que o agente não consegue forjar |
...
- [Martin Fowler / Birgitta Böckeler, Harness engineering for coding agente users](https://martinfowler.com/articles/harness-engineering.html) — guides (feedforward) + sensors (feedback); a formulação de teoria de controle mais limpa
...
- [HKUDS/OpenHarness](https://github.com/HKUDS/OpenHarness) — agente harness open-source com agente pessoal embutido

Threads do Hacker News vale a pena ler pelas discordâncias, não pelo consenso:

- [HN: Effective harnesses for long-running agents](https://news.ycombinator.com/item?id=46081704)
- [HN: Improving 15 LLMs at Coding in One Afternoon. Only the Harness Changed](https://news.ycombinator.com/item?id=46988596)
- [HN: The agente harness belongs outside the sandbox](https://news.ycombinator.com/item?id=47990675) — argumenta que autorização é um plano separado
